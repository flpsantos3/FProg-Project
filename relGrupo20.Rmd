...
```
Foram também incluídas duas exceções extra, para exceções do tipo `FileNotFoundError` e `TypeError`, que são levantadas quando um dos ficheiros de entrada não existe ou quando é introduzido um *input* que não seja um ficheiro .txt.
```{r eval=FALSE}
try:
